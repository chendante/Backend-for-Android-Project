...
func StuSelectRegister(sid uint) (RegisterInfo, bool) {
	...
	var ok bool
	Db.Table("StuLesson").Select("Lesson.lesson_name, Register.rid, Register.lid, Register.begin_time").Joins("inner join Lesson").Joins("inner join Register").Where("Register.delete_status = ? AND StuLesson.sid = ?", 1, sid).Last(&register)
	if register.Rid == 0{
		ok = false
	} else {
